...
class PdParser {
	...
	public:
		...
		typedef std::vector<std::string> AtomLine;
		...
		static void getAtomLines(string patchText, vector<AtomLine>& atomLines) {
			...
			Poco::RegularExpression pattern(line_re, Poco::RegularExpression::RE_MULTILINE);
	...
	private:
		...
		static const string line_re;
};

const string PdParser::line_re = "(#((.|\r|\n)*?)[^\\\\])\r{0,1}\n{0,1};\r{0,1}\n";
